  ...
  <div>
    <v-row justify="space-between">
      <v-col cols="6" lg="4" md="6">
        <v-text-field v-model="searchText" placeholder="Pesquisa por nome"></v-text-field>
      </v-col>
      <v-col cols="2">
        <v-btn @click.native="changeSortOrder()">
    ...
    <v-row>
      <v-col v-if="$vuetify.breakpoint.xs">
        <v-btn block> Pedir materiais </v-btn>
      </v-col>
      <v-col v-else cols="4" offset="8">
        <v-btn block> Pedir materiais </v-btn>
      </v-col>
    </v-row>
  </div>
