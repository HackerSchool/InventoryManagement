  ...
  <v-row>
    ...
    <v-col>
      ...
      <v-row align="center" no-gutters>
        <v-col>
          <v-row> Em stock: {{ quantity }} </v-row>
        </v-col>
        <v-col>
          <v-row justify="center" align="center">
            A pedir: &nbsp;
            <b style="font-size: 150%; text-decoration: underline"> {{ askedQuantity }} </b>
          </v-row>
        </v-col>
      </v-row>
    </v-col>
...
export default {
  ...
  methods: {
    addQuantity(howMuch) {
      ...
      console.log('test');
      if (wantedQuantity + howMuch < 0 || wantedQuantity + howMuch > this.quantity) {
        return;
      }
      console.log('test2');
