...
export default {
  ...
  methods: {
    ...
    fetchCreators() {
      api
        .post('/search/creators-autocomplete', {
          name: '',
        })
        .then((creatorsResponse) => {
          for (var i = 0; i < creatorsResponse.data.length; i++) {
            let name = [];
            if (creatorsResponse.data[i].first_name) name.push(creatorsResponse.data[i].first_name);
            if (creatorsResponse.data[i].last_name) name.push(creatorsResponse.data[i].last_name);
